...
//    fn.AppendDir(wxT("ais_ids_pi"));
//#else
//    fn.SetPath(*GetpSharedDataLocation());
    //const char *sPluginName = "ais_ids_pi";
    //fn.SetPath(GetPluginDataDir(sPluginName));
    //const char *sPluginName = "ais_ids_pi";
    fn.SetPath(GetPluginDataDir("ais_ids_pi"));
    //    fn.AppendDir( wxT("plugins") );
//    fn.AppendDir(wxT("ais_ids_pi"));
    fn.AppendDir(wxT("data"));
    g_SData_Locn = new wxString(fn.GetFullPath().c_str());
//#endif
    wxString s = _("ais_ids_pi data location");
    wxLogMessage( wxT("%s: %s"), s.c_str(), fn.GetFullPath().c_str());

    m_failedBitmapLoad = false;

#ifdef PLUGIN_USE_SVG
    fn.SetFullName(wxT("ais_ids.svg"));
    m_s_ais_ids_pi = fn.GetFullPath();
    m_bm_ais_ids_pi = LoadSVG( fn.GetFullPath() );
    fn.SetFullName(wxT("ais_idsgrey.svg"));
    m_s_ais_ids_grey_pi = fn.GetFullPath();
    m_bm_ais_ids_grey_pi = LoadSVG( fn.GetFullPath() );
    fn.SetFullName(wxT("ais_ids-toggled.svg"));
    m_s_ais_ids_toggled_pi = fn.GetFullPath();
    m_bm_ais_ids_toggled_pi = LoadSVG( fn.GetFullPath() );
#else
    fn.SetFullName(wxT("ais_ids.png"));
    m_p_bm_ais_ids_pi = new wxBitmap( fn.GetFullPath(), wxBITMAP_TYPE_PNG );
    if(!m_p_bm_ais_ids_pi->IsOk())  m_failedBitmapLoad = true;
#endif

    if(m_failedBitmapLoad) {
        int ret = OCPNMessageBox_PlugIn( NULL, _("Failed to load all ais_ids_pi icons, check OCPN log for details"), _("OpenCPN Alert"), wxOK );
    } else {
        CreateSchemeIcons();
        ScaleIcons();
    }
...
wxBitmap tpicons::LoadSVG( const wxString filename, unsigned int width, unsigned int height )
{
    ...
    wxString s = _("ais_ids_pi LoadSVG");
...
bool tpicons::ScaleIcons()
{
    ...
    //m_bm_ais_ids_pi = ScaleIcon( m_p_svgd_ais_ids_pi, m_p_img_ais_ids_pi, m_dScaleFactor );
    //m_bm_ais_ids_grey_pi = ScaleIcon( m_p_svgd_ais_ids_grey_pi, m_p_img_ais_ids_grey_pi, m_dScaleFactor );
...
void tpicons::ChangeScheme(void)
{
    switch(m_ColourScheme) {
        ...
        case PI_GLOBAL_COLOR_SCHEME_DAY:
            m_bm_ais_ids_grey_pi = m_bm_day_ais_ids_grey_pi;
            break;
        case PI_GLOBAL_COLOR_SCHEME_DUSK:
            m_bm_ais_ids_grey_pi = m_bm_dusk_ais_ids_grey_pi;
            break;
        case PI_GLOBAL_COLOR_SCHEME_NIGHT:
            m_bm_ais_ids_grey_pi = m_bm_night_ais_ids_grey_pi;
            break;
        default:
            break;
    }
...
{
    m_bm_day_ais_ids_grey_pi = m_bm_ais_ids_grey_pi;
    m_bm_day_ais_ids_toggled_pi = m_bm_ais_ids_toggled_pi;
    m_bm_day_ais_ids_pi = m_bm_ais_ids_pi;
    m_bm_dusk_ais_ids_grey_pi = BuildDimmedToolBitmap(m_bm_ais_ids_grey_pi, 128);
    m_bm_dusk_ais_ids_pi = BuildDimmedToolBitmap(m_bm_ais_ids_pi, 128);
    m_bm_dusk_ais_ids_toggled_pi = BuildDimmedToolBitmap(m_bm_ais_ids_toggled_pi, 128);
    m_bm_night_ais_ids_grey_pi = BuildDimmedToolBitmap(m_bm_ais_ids_grey_pi, 32);
    m_bm_night_ais_ids_pi = BuildDimmedToolBitmap(m_bm_ais_ids_pi, 32);
    m_bm_night_ais_ids_toggled_pi = BuildDimmedToolBitmap(m_bm_ais_ids_toggled_pi, 32);
