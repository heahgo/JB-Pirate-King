...
static void DecodePayloadBits(const wxString &payload, int fill_bits,
                              std::vector<int> &bits)
{
    bits.clear();
    bits.reserve(payload.Length() * 6);
    for (size_t i = 0; i < payload.Length(); ++i) {
        int v = SixBitValue(static_cast<char>(payload[i]));
        for (int b = 5; b >= 0; --b) {
            bits.push_back((v >> b) & 1);
        }
    }
    while (fill_bits-- > 0 && !bits.empty()) {
        bits.pop_back();
    }
}

static int GetBits(const std::vector<int> &bits, int start, int len)
{
    int v = 0;
    for (int i = 0; i < len && (start + i) < static_cast<int>(bits.size()); ++i) {
        v = (v << 1) | bits[start + i];
    }
    return v;
}

static int SignedValue(int v, int bit_len)
{
    int sign = 1 << (bit_len - 1);
    if (v & sign) {
        v -= (1 << bit_len);
    }
    return v;
...
static wxString AisSixBitToText(const std::vector<int> &bits, int start, int chars)
{
    wxString out;
    out.reserve(chars);
    for (int i = 0; i < chars; ++i) {
        int val = GetBits(bits, start + i * 6, 6);
        char ch = (val < 32) ? static_cast<char>(val + 64)
...
static bool ParseAisSentence(const wxString &sentence, AisSummary &out,
                             std::unordered_map<int, wxString> &name_cache)
{
    wxString core = sentence.BeforeFirst('*');
    wxStringTokenizer tk(core, ",");
    std::vector<wxString> fields;
    while (tk.HasMoreTokens()) {
        fields.push_back(tk.GetNextToken());
    }
    if (fields.size() < 7) return false;
    if (!fields[0].StartsWith("!AIVDM") && !fields[0].StartsWith("!AIVDO")) return false;

    long total = 0, num = 0, fill = 0;
    fields[1].ToLong(&total);
    fields[2].ToLong(&num);
    if (total != 1 || num != 1) return false;

    wxString payload = fields[5];
    wxString fill_str = fields[6];
    fill_str = fill_str.BeforeFirst('*');
    fill_str.ToLong(&fill);

    std::vector<int> bits;
    DecodePayloadBits(payload, static_cast<int>(fill), bits);
    if (bits.size() < 40) return false;

    int msg_type = GetBits(bits, 0, 6);
    int mmsi = GetBits(bits, 8, 30);
    out.mmsi = mmsi;

    if (msg_type == 1 || msg_type == 2 || msg_type == 3) {
        int sog_raw = GetBits(bits, 50, 10);
        int lon_raw = SignedValue(GetBits(bits, 61, 28), 28);
        int lat_raw = SignedValue(GetBits(bits, 89, 27), 27);
        int cog_raw = GetBits(bits, 116, 12);

        out.sog = (sog_raw == 1023) ? NAN : (sog_raw / 10.0);
        out.lon = (std::abs(lon_raw) >= 181 * 600000) ? NAN : (lon_raw / 600000.0);
        out.lat = (std::abs(lat_raw) >= 91 * 600000) ? NAN : (lat_raw / 600000.0);
        out.cog = (cog_raw == 3600) ? NAN : (cog_raw / 10.0);

        auto it = name_cache.find(mmsi);
        if (it != name_cache.end()) out.name = it->second;
        return true;
    }

    if (msg_type == 24) {
        int part = GetBits(bits, 38, 2);
        if (part == 0) {
            wxString name = AisSixBitToText(bits, 40, 20);
            if (!name.IsEmpty()) {
...
void ais_ids_pi::SetAISSentence(wxString &sentence) {
    ...
    if (!m_tpControlDialogImpl || !m_tpControlDialogImpl->IsShown()) return;
    static std::unordered_map<int, wxString> name_cache;
    AisSummary summary;
    if (!ParseAisSentence(sentence, summary, name_cache)) {
        m_tpControlDialogImpl->AppendAisSentence("PARSE_FAIL: " + sentence);
        return;
    }
    wxString lat = std::isnan(summary.lat) ? "NA" : wxString::Format("%.5f", summary.lat);
    wxString lon = std::isnan(summary.lon) ? "NA" : wxString::Format("%.5f", summary.lon);
    wxString sog = std::isnan(summary.sog) ? "NA" : wxString::Format("%.2f", summary.sog);
    wxString cog = std::isnan(summary.cog) ? "NA" : wxString::Format("%.1f", summary.cog);
    wxString name = summary.name.IsEmpty() ? "-" : summary.name;

    m_tpControlDialogImpl->AppendAisSentence(wxString::Format(
        "MMSI=%d Lat=%s Lon=%s SOG=%s COG=%s Name=%s",
        summary.mmsi, lat, lon, sog, cog, name));
}
