...
void tpJSON::ProcessMessage(wxString &message_id, wxString &message_body)
{
    ...
    if(g_ais_ids_pi->m_bSaveIncommingJSONMessages) {
        if(!m_ffOutputFile) {
            wxString l_mode;
            if(g_ais_ids_pi->m_bAppendToSaveFile) l_mode = "a";
            else l_mode = "w";
            m_ffOutputFile  = new wxFFile(g_ais_ids_pi->m_fnOutputJSON.GetFullPath(), l_mode);
            if(!m_ffOutputFile->IsOpened()) {
                g_ais_ids_pi->m_bSaveIncommingJSONMessages = false;
                OCPNMessageBox_PlugIn(NULL, g_ais_ids_pi->m_fnOutputJSON.GetPath(), _("File not found"), wxICON_EXCLAMATION | wxCANCEL);
            }
        }
        ...
        if(g_ais_ids_pi->m_bCloseSaveFileAfterEachWrite) {
            m_ffOutputFile->Flush();
            m_ffOutputFile->Close();
            delete m_ffOutputFile;
            m_ffOutputFile = NULL;
    ...
    if(message_id != _T("AIS_IDS_PI")) {
        ...
        if ( numErrors > 0 )  {
            ...
            for(size_t i = 0; i < errors.GetCount(); i++)
            {
                if(i == 0) {
                    sLogMessage.clear();
                    sLogMessage.Append(wxT("ais_ids_pi: Error parsing JSON message - "));
        ...
        if(!bFail && root[wxS("Msg")].AsString() == wxS("Version") && root[wxS("Type")].AsString() == wxS("Request")) {
            jMsg[wxT("Source")] = wxT("AIS_IDS_PI");
